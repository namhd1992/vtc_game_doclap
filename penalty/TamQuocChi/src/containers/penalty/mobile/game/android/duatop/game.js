...
export default class Game extends Phaser.Scene{
    ...
    create(){
        ...
        this.txt_title = this.add.text(Math.round(520*delta_x),  Math.round(10*delta_y), "ĐUA TOP", { font: `${Math.round(40*delta_x)}px Arial`, fill: "#ffffff", align:'center' });
        ...
        this.txt_points = this.add.text(Math.round(975*delta_x),  Math.round(45*delta_y), `Điểm: 00`, { font: `${Math.round(18*delta_x)}px Arial`, fill: "#ffffff", align:'center' });
            ...
            if(points>0){
                if(p1[1]-p2[1] > 0){
                    ...
                    if(user!==null){
                        ...
                        axios.post(Ultilities.base_url() +'/tqc_lobby/api/v1/race/playing', data, header).then(function (response) {
    ...
    showThoat(text) {
        ...
        this.back = this.add.sprite(Math.round(600*delta_x), Math.round((675/2)*delta_y), "bg_pop_ingame");
        this.back.setScale(delta_x,delta_y)
        this.thoatButton = this.add.sprite(Math.round(width/2), Math.round(480*delta_y), "btn_thoat");
        this.thoatButton.setScale(delta_x,delta_y)
        this.text1 = this.add.text(Math.round(400*delta_x), Math.round(300*delta_y), text, { font: `${Math.round(18*delta_x)}px Arial`, fill: "#ffffff", align:'center', fixedWidth: Math.round(400*delta_x), wordWrap:true});
        this.thoatButton.setInteractive().on(Phaser.Input.Events.GAMEOBJECT_POINTER_DOWN, ()=>{
            window.location.replace('/')
    ...
    loadInitData=()=>{
        ...
        if(info_seesion!==null){
            if(user!==null){
                ...
                axios.post(Ultilities.base_url() +'/tqc_lobby/api/v1/race/connect', data, header).then(function (response) {
    ...
    updateData=()=>{
        ...
        if(user!==null){
            ...
            axios.post(Ultilities.base_url() +'/tqc_lobby/api/v1/race/state', data, header).then(function (response) {
                if(response.data !==undefined){
                    if(response.data.code>=0){
    ...
    finishGame=()=>{
        ...
        if(user!==null){
            ...
            axios.post(Ultilities.base_url() +'/tqc_lobby/api/v1/race/summary', data, header).then(function (response) {
                if(response.data !==undefined){
                    if(response.data.code>=0){
