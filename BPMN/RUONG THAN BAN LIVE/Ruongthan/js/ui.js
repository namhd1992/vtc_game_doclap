const game_client = {
	...
	setDataToUI(response){
		...
		var obj_gia_goi1=data.filter(v =>v.code===contants.EVT_GIA_GOI_1);
		var obj_gia_goi2=data.filter(v =>v.code===contants.EVT_GIA_GOI_2);
		var obj_gia_goi3=data.filter(v =>v.code===contants.EVT_GIA_GOI_3);
		var obj_gia_goi4=data.filter(v =>v.code===contants.EVT_GIA_GOI_4);
		var obj_gia_goi5=data.filter(v =>v.code===contants.EVT_GIA_GOI_5);
		var obj_gia_goi6=data.filter(v =>v.code===contants.EVT_GIA_GOI_6);
		var obj_gia_goi_vip=data.filter(v =>v.code===contants.EVT_GIA_GOI_VIP);
		var obj_gia_goi_thuong=data.filter(v =>v.code===contants.EVT_GIA_GOI_THUOG);
		...
		game_client.goi_1=obj_gia_goi1[0] ? obj_gia_goi1[0].value : 0;
		game_client.goi_2=obj_gia_goi2[0] ? obj_gia_goi2[0].value : 0;
		game_client.goi_3=obj_gia_goi3[0] ? obj_gia_goi3[0].value : 0;
		game_client.goi_4=obj_gia_goi4[0] ? obj_gia_goi4[0].value : 0;
		game_client.goi_5=obj_gia_goi5[0] ? obj_gia_goi5[0].value : 0;
		game_client.goi_6=obj_gia_goi6[0] ? obj_gia_goi6[0].value : 0;
		game_client.goi_vip=obj_gia_goi_vip[0] ? obj_gia_goi_vip[0].value : 0;
		game_client.goi_thuong=obj_gia_goi_thuong[0] ? obj_gia_goi_thuong[0].value : 0;
	...
	topupsScoinToPoint(){
		...
		if(vtcmAuth.isLogin()){
            vtcmEvent.topupsScoinToPoint(data, this.handlingTopupsScoinToPoint, this.notification)
        } else{
            game_client.showLogin();
        }
	},
	...
	handlingPurchases(res, objectParamsReturn){
		console.log('game_client.goi_1', game_client.goi_1)
		if(res.data.code >= 0){
